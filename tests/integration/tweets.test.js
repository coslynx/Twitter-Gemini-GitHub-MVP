...
describe('Tweet Integration Tests', () => {
  ...
  it('should successfully process and store tweets', async () => {
    const res = await request(server).post('/tweets').send({ keywords: 'test' });
  ...
  it('should handle Twitter API rate limit', async () => {
    const mockTwitterService = {
      fetchTweets: jest.fn().mockRejectedValue(new Error('Rate limit exceeded')),
    };
  ...
  it('should handle Gemini API error', async () => {
    const mockGeminiService = {
      generateMarkdown: jest.fn().mockRejectedValue(new Error('Gemini API error')),
    };
  ...
  it('should handle database error', async () => {
    jest.spyOn(Tweet, 'insertMany').mockRejectedValue(new Error('Database error'));
  ...
  it('should handle GitHub API error', async () => {
    const mockGithubService = {
      uploadMarkdownFile: jest.fn().mockResolvedValue({ success: false, message: 'GitHub API error' }),
    };
  ...
  it('should handle empty keywords and hashtags', async () => {
    const res = await request(server).post('/tweets').send({ keywords: '', hashtags: '' });
    expect(res.status).toBe(400); // Expect a bad request error code
    expect(res.body.error).toBeDefined();
  });

  it('should handle non-string keywords', async () => {
    const res = await request(server).post('/tweets').send({ keywords: 123, hashtags: 'test' });
    expect(res.status).toBe(400); // Expect a bad request error code
    expect(res.body.error).toBeDefined();
  });
  ...
  it('should handle non-string hashtags', async () => {
    const res = await request(server).post('/tweets').send({ keywords: 'test', hashtags: 123 });
    expect(res.status).toBe(400); // Expect a bad request error code
    expect(res.body.error).toBeDefined();
  });
});
