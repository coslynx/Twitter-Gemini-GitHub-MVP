...
describe('GitHub Integration Tests', () => {
  ...
  it('should handle GitHub API rate limit exceeded', async () => {
    GithubService.uploadMarkdownFile.mockRejectedValue({ status: 429, message: 'GitHub API rate limit exceeded' });
  ...
  it('should handle file not found error', async () => {
    GithubService.uploadMarkdownFile.mockRejectedValue({ code: 'ENOENT', message: 'File not found' });
    const response = await supertest(server)
      .post('/github/upload')
      .expect(400);
  ...
  it('should handle generic server error', async () => {
    GithubService.uploadMarkdownFile.mockRejectedValue({ status: 500, message: 'Internal Server Error' });
