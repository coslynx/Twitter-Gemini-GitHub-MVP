...
describe('TwitterService', () => {
  ...
  describe('fetchTweets', () => {
    ...
    it('should handle invalid inputs', async () => {
      expect(() => service.fetchTweets()).toThrow('Keywords or hashtags are required.');
    });
    it('should handle pagination', async () => {
      ...
      service.client.v2.search.mockResolvedValueOnce(mockResponsePage1).mockResolvedValueOnce(mockResponsePage2);
      const tweets = await service.fetchTweets('test');
    ...
    it('should handle rate limit exceeded', async () => {
      ...
      await expect(service.fetchTweets('test')).rejects.toThrow('Twitter API rate limit exceeded');
    });
    it('should handle network error', async () => {
      const mockError = { code: 'ETIMEDOUT' };
      service.client.v2.search.mockRejectedValue(mockError);
      await expect(service.fetchTweets('test')).rejects.toThrow('Request timed out');
    });
    it('should handle authentication error', async () => {
      const mockError = { message: 'Unauthorized' };
      service.client.v2.search.mockRejectedValue(mockError);
      await expect(service.fetchTweets('test')).rejects.toThrow('Failed to fetch tweets');
    });
