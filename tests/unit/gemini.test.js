  ...
  describe("generateMarkdown", () => {
    it("should successfully generate Markdown for a single tweet", async () => {
      ...
      expect(result).toEqual([
        { id: "123", text: "Test tweet", markdown: "# Test tweet" },
      ]);
      expect(mockGeminiClient.generateText).toHaveBeenCalledWith({
        prompt: expect.stringContaining(JSON.stringify(singleTweet)),
      });
    });
    ...
    it("should handle multiple tweets", async () => {
      ...
      const mockResponses = [{ text: "# Tweet 1" }, { text: "# Tweet 2" }];
    ...
    it("should handle Gemini API error", async () => {
      ...
      expect(result).toEqual([
        { id: "1", text: "Tweet 1", markdown: "Markdown generation failed" },
      ]);
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining("Error generating Markdown for tweet: 1"),
    ...
    it("should handle malformed tweet data", async () => {
      const tweet = {
        id: "1",
        text: "test",
        entities: { urls: [{ url: "http://example.com", expanded_url: null }] },
      };
      mockGeminiClient.generateText.mockResolvedValue([
      ...
      expect(result).toEqual([
        {
          ...
          text: "test",
          entities: {
            urls: [{ url: "http://example.com", expanded_url: null }],
          },
    ...
    it("should handle a tweet with a long text", async () => {
      ...
      mockGeminiClient.generateText.mockResolvedValue([
        { text: "Markdown summary of long text" },
      ]);
      const result = await service.generateMarkdown([tweet]);
      expect(result).toEqual([
        { id: "1", text: longText, markdown: "Markdown summary of long text" },
      ]);
    });
    ...
    it("should handle a tweet with special characters", async () => {
      const tweet = {
        id: "1",
        text: "This tweet has special characters: !@#$%^&*()_+=-`~[]{}|;':\",./<>? ",
      };
      mockGeminiClient.generateText.mockResolvedValue([
        { text: "Markdown summary" },
      ]);
