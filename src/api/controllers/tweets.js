...
class TweetsController {
  ...
  async fetchAndProcessTweets(keywords, hashtags) {
    ...
    const query = `(${keywords ? keywords : ''}) ${hashtags ? hashtags : ''}`.trim();
  ...
  async saveTweets(tweets) {
    try {
      const existingTweets = await this.tweetModel.find({ id: { $in: tweets.map((t) => t.id) } });
      const newTweets = tweets.filter((tweet) => !existingTweets.find((t) => t.id === tweet.id));
