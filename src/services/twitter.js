...
class TwitterService {
  ...
  parseMetric(value) {
    ...
    const num = parseFloat(
      value.replace(/[,K]/g, (match) => (match === "K" ? "000" : ""))
    );
  ...
  async findAndProcessThreads() {
    const MAX_THREADS = 5;
    const MIN_LINKS_PER_THREAD = 3;
    let processedThreads = 0;
    let allLinks = [];
    let processedUrls = new Set();

    while (processedThreads < MAX_THREADS) {
      try {
        const threadStarter = await this.findThreadStarter(processedUrls);
        if (!threadStarter) {
          logger.info("No more thread starters found");
          break;
        }

        const threadLinks = await this.processFullThread(threadStarter);

        if (threadLinks.length >= MIN_LINKS_PER_THREAD) {
          allLinks = allLinks.concat(threadLinks);
          processedThreads++;
          logger.info(
            `Processed thread ${processedThreads}/${MAX_THREADS} with ${threadLinks.length} links`
          );
        } else {
          logger.info(
            `Skipped thread with insufficient links (${threadLinks.length})`
          );
        }

        if (allLinks.length >= 10) {
          logger.info("Reached target number of links");
          break;
        }
  ...
  async findThreadStarter(processedUrls) {
    try {
      const threadStarters = await this.page.evaluate(() => {
        const tweets = Array.from(
          document.querySelectorAll('article[data-testid="tweet"]')
        ...
        return tweets
          .map((tweet) => {
            ...
            const hasStrongThreadIndicator =
              text?.includes("🧵") ||
              text?.toLowerCase().includes("thread:") ||
              /1\/\d+|thread|part 1 of|step 1 of|\(1\/\d+\)/i.test(text);
            ...
            const hasFollowUpTweets =
              tweet.parentElement?.querySelectorAll(
                'article[data-testid="tweet"]'
              ).length > 2;

            const isValidThread =
              hasStrongThreadIndicator ||
              (hasThreadStructure && hasFollowUpTweets);
          ...
          .filter(
            (tweet) => tweet && tweet.url && !processedUrls.has(tweet.url)
          );
      });

      if (!threadStarters.length) {
        return null;
      }

      const threadStarter = threadStarters[0];
      processedUrls.add(threadStarter.url);

      const tweetCount = await this.page.evaluate(() => {
        const authorHandle = document
          .querySelector(
            'article[data-testid="tweet"] div[data-testid="User-Name"] a'
          )
          ?.textContent?.trim();
        return Array.from(
          document.querySelectorAll('article[data-testid="tweet"]')
      ...
      if (tweetCount < 3) {
        logger.info(
          `Skipping non-thread tweet (only ${tweetCount} tweets found): ${threadStarter.url}`
        );
        return null;
      }

      logger.info("Found thread starter:", {
        text: threadStarter.text,
        url: threadStarter.url,
        tweetCount,
      });

      return threadStarter;
    } catch (error) {
      logger.error("Error finding thread starter:", error);
      return null;
    }
  }

  async processFullThread(threadStarter) {
    try {
      await this.page.goto(threadStarter.url, {
        waitUntil: "networkidle0",
        timeout: 30000,
      });
      ...
      await this.scrollThreadToEnd();

      const threadContent = await this.page.evaluate(() => {
        const tweets = Array.from(
          document.querySelectorAll('article[data-testid="tweet"]')
        );
        ...
        return tweets.map((tweet) => {
          const tweetText = tweet
            .querySelector('[data-testid="tweetText"]')
            ?.innerText?.trim();
          const links = Array.from(tweet.querySelectorAll('a[href^="http"]'))
            .map((link) => {
              try {
      ...
      const threadLinks = threadContent
        .filter((tweet) => tweet.links.length > 0)
        .map((tweet) => ({
          threadContext: threadStarter.text,
          tweetText: tweet.text,
          links: tweet.links,
        }));

      logger.info(`Found ${threadLinks.length} tweets with links in thread`);
      return threadLinks;
    } catch (error) {
      logger.error(`Error processing thread ${threadStarter.url}:`, error);
      return [];
    }
  }
}
